...
///    \code
///      cc::Logging::MessageBuilder::Ref msg = create_log_message(status::Level::LEVELNAME);
///      *msg << arg << ... ;
///      ...
///      msg->submit();
///    \endcode
...
#define logf_notice(...)         logf_message(cc::status::Level::NOTICE, __VA_ARGS__)
#define logf_warning(...)        logf_message(cc::status::Level::WARNING, __VA_ARGS__)
#define logf_error(...)          logf_message(cc::status::Level::FAILED, __VA_ARGS__)
#define logf_critical(...)       logf_message(cc::status::Level::CRITICAL, __VA_ARGS__)
#define logf_fatal(...)          logf_message(cc::status::Level::FATAL, __VA_ARGS__)
...
#define assertf(cond, ...)                                                           \
    if (!(cond))                                                                     \
    {                                                                                \
        logf_message(cc::status::Level::FATAL, "Assertion failed: "s + __VA_ARGS__); \
        throw std::terminate;                                                        \
    }
#else
#define assertf(cond, ...)
