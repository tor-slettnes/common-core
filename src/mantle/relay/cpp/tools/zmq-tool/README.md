...
The `relay-dds-tool` executable provides end-user access to our relay API. It incorporates both of the [API implementations](../../impl/README.md) from our relay by linking in the following libraries:

* [cc_relay_native](../../impl/native/README.md), the core library
* [cc_relay_dds_subscriber](../../impl/dds/dds-subscriber), containing the DDS subscriber
* [cc_relay_dds_client](../../impl/dds/rpc-client), containing the RPC client
 ...
 * In a dedicated terminal tab/window, monitor greetings from others via `RelayService` by invoking:

   ```shell
   $ relay-dds-tool monitor greetings
 ...
 * Open a new terminal window and send a greeting via the Pub/Sub backend:
   ...
   ```shell
   $ relay-dds-tool say_hello "I am here!"
   ```

   In the first terminal you should now see a message similar to this:

   ```shell
   Greeting(text="I am here!", identity="relay-dds-tool", implementation="dds", birth=birth=2023-05-09@00:41:41.029, uptime=0.000 sec)
   ```
