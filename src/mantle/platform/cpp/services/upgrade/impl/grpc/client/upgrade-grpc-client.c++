// -*- c++ -*-
//==============================================================================
/// @file upgrade-grpc-client.c++
/// @brief Upgrade gRPC client
/// @author Tor Slettnes
//==============================================================================

#include "upgrade-grpc-client.h++"
#include "upgrade.h++"
#include "protobuf-upgrade-types.h++"
#include "protobuf-inline.h++"

namespace cc::platform::upgrade::grpc
{
    void Client::initialize()
    {
        Super::initialize();

        this->add_handler(
            platform::upgrade::protobuf::Signal::kScanProgress,
            &This::on_scan_progress);

        this->add_handler(
            platform::upgrade::protobuf::Signal::kUpgradeAvailable,
            &This::on_upgrade_available);

        this->add_handler(
            platform::upgrade::protobuf::Signal::kUpgradePending,
            &This::on_upgrade_pending);

        this->add_handler(
            platform::upgrade::protobuf::Signal::kUpgradeProgress,
            &This::on_upgrade_progress);
    }

    void Client::on_scan_progress(const platform::upgrade::protobuf::Signal &signal)
    {
        upgrade::ScanProgress::ptr progress;
        if (This::is_mapped(signal.mapping_action()))
        {
            cc::protobuf::decode_shared(signal.scan_progress(), &progress);
        }
        upgrade::signal_scan_progress.emit(progress);
    }

    void Client::on_upgrade_available(const platform::upgrade::protobuf::Signal &signal)
    {
        upgrade::PackageInfo::ptr available_info;
        if (This::is_mapped(signal.mapping_action()))
        {
            cc::protobuf::decode_shared(signal.upgrade_available(), &available_info);
        }
        upgrade::signal_upgrade_available.emit(available_info);
    }

    void Client::on_upgrade_pending(const platform::upgrade::protobuf::Signal &signal)
    {
        upgrade::PackageInfo::ptr pending_info;
        if (This::is_mapped(signal.mapping_action()))
        {
            cc::protobuf::decode_shared(signal.upgrade_pending(), &pending_info);
        }
        upgrade::signal_upgrade_pending.emit(pending_info);
    }

    void Client::on_upgrade_progress(const platform::upgrade::protobuf::Signal &signal)
    {
        upgrade::UpgradeProgress::ptr progress;
        if (This::is_mapped(signal.mapping_action()))
        {
            cc::protobuf::decode_shared(signal.upgrade_progress(), &progress);
        }
        upgrade::signal_upgrade_progress.emit(progress);
    }

}  // namespace cc::platform::upgrade::grpc
