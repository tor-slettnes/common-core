...
{
    std::cout << platform::network::network->get_global_data() << std::endl;
}

void Options::get_devices()
{
    for (const auto &kv : platform::network::network->get_devices())
    {
...
{
    for (const auto &kv : platform::network::network->get_connections())
    {
        core::str::format(std::cout, "%10s: %s\n", kv.first, kv.second);
    }
}
...
{
    std::string name = this->get_arg("connection name");
    platform::network::network->remove_connection(name);
}

void Options::activate_connection()
{
    std::string name = this->get_arg("connection name");
    platform::network::network->activate_connection(name);
}

void Options::get_active_connections()
{
    for (const auto &[name, conn] : platform::network::network->get_active_connections())
    {
...
{
    platform::network::network->request_scan();
}

void Options::get_aps()
{
    for (const auto &[name, ap] : platform::network::network->get_aps())
    {
...
void Options::set_wireless_allowed()
{
    ...
    platform::network::network->set_wireless_allowed(core::str::convert_to<bool>(allowed));
}

void Options::set_wireless_enabled()
{
    std::string enabled = this->get_arg("boolean value");
    platform::network::network->set_wireless_enabled(core::str::convert_to<bool>(enabled));
}
