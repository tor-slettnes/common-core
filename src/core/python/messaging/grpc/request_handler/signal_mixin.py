'''
signal_mixin.py - gRPC request handler mix-in with `Watch` method to feed back Signal messages
'''
...
class SignalMixIn:
    '''
    gRPC request handler mix-in with `Watch()` method to stream signals back
    to client.
    '''
