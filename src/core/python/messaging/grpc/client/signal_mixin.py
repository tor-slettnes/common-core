'''
signal_mixin.py - Client mix-in for gRPC services with a `Signal` message
...
In your `SignalMixIn` subclass you would do one of the following:
 ...
 * Pass an existing `SignalStore()` instance to `__init__()`:
   ...
   from cc.messaging.grpc import GenericClient, SignalMixIn

   class MyServiceClient (SignalMixIn, GenericClient):
       from cc.protobuf.core.my_service_pb2_grpc import MyServiceStub as Stub

       def __init__ (self, *args, **kwargs):
           SignalMixIn.__init__(self, signal_store = my_signal_store)
           GenericClient.__init__(self, *args, **kwargs)
...
@example ../../../../mantle/demo/python/grpc/client.py
'''
...
class SignalMixIn:
    '''
    gRPC client mix-in that adds functionality to receive streamed Signal
    ...
    def __init__(self,
        ...
        if signal_store:
            ...
            assert signal_type is not None, (
                'SignalMixIn() subclass %s() must either pass in a '
